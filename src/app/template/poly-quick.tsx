...
import { INK, useSliderTrack, useThemedInk, getContrastColor } from '@/components/templates/primitives';
...
function MeterRow({ m, value, onChange, editing }: {
  ...
  return (
    <View style={qs.meterRow}>
      <Text style={[qs.meterL, { color: ink }]}>{m.l}</Text>
      <View ref={trackRef} style={qs.meterTrack} {...responder}>
        <View style={[qs.meterBase, { borderColor: ink }]} />
        <View style={[qs.meterFill, { width: `${value * 100}%` as any, backgroundColor: ink }]} />
        <View style={[qs.meterThumb, { left: `${value * 100}%` as any, borderColor: ink }]} />
      </View>
      <Text style={[qs.meterR, { color: ink }]}>{m.r}</Text>
    </View>
  );
}
...
function PolyQuickContent({ editing, shipId }: { editing?: boolean; shipId?: string }) {
  ...
  return (
    <View style={[qs.card, { borderColor: ink }, tBg]}>
      <View style={qs.headerWrap}>
        <View style={[qs.titlePill, { backgroundColor: ink }]}><Text style={[qs.titlePillText, { color: getContrastColor(ink) }]}>OUR POLYSHIP IN 5 MINUTES</Text></View>
        <Text style={[qs.subtitle, { color: ink }]}>the quick version ♡  ·  tap anything to edit</Text>
      </View>
      ...
      <View style={qs.shipRow}>
        <Labeled label="Ship">
          <TextInput value={ship?.shipName ?? ''} editable={e} onChangeText={(v) => shipId && updateShip(shipId, { shipName: v })}
            placeholder="name…" placeholderTextColor={ink + '77'} style={[qs.lineInput, { color: ink, borderColor: ink }]} />
        </Labeled>
        <Labeled label="Media">
          <TextInput value={ship?.fandom ?? ''} editable={e} onChangeText={(v) => shipId && updateShip(shipId, { fandom: v })}
            placeholder="fandom…" placeholderTextColor={ink + '77'} style={[qs.lineInput, { color: ink, borderColor: ink }]} />
        </Labeled>
      </View>
      ...
      <View style={qs.chipsWrap}>
        {roster.map((m) => (
          <View key={m.id} style={[qs.memberChip, { borderColor: ink }, tBg]}>
            <View style={[qs.chipDot, { backgroundColor: m.color, borderColor: ink }]} />
            {e ? (
              <TextInput value={m.name} onChangeText={(v) => setMemberName(m.id, v)} placeholder="name"
                placeholderTextColor={ink + '77'} style={[qs.chipInput, { color: ink }]} />
            ) : (
              <Text style={[qs.chipName, { color: ink }]}>{m.name || '—'}</Text>
            )}
            {e && roster.length > 2 && (
        ...
        {e && (
          <Pressable style={[qs.addChip, { backgroundColor: ink }]} onPress={addMember}><Text style={[qs.addChipText, { color: getContrastColor(ink) }]}>＋ add</Text></Pressable>
        )}
      </View>
      ...
      <View style={qs.wordsRow}>
        {[0, 1, 2].map((i) => (
          <TextInput key={i} value={words[i] ?? ''} editable={e}
            ...
            placeholder="word…" placeholderTextColor={ink + '77'} style={[qs.wordInput, { color: ink, borderColor: ink }]} />
        ))}
      </View>
        ...
        {ROLE_LABELS.map((label, i) => {
          ...
          return (
            <Pressable key={rk} style={qs.roleRow} onPress={e ? () => cycleRole(rk) : undefined} disabled={!e}>
              <Text style={[qs.roleLabel, { color: ink }]}>{label}</Text>
              <View style={qs.roleDotted} />
              {v === 'all' ? (
                <Text style={qs.roleAll}>all of them ♡</Text>
              ) : m ? (
                <View style={[qs.roleTag, { backgroundColor: m.color, borderColor: ink }]}><Text style={qs.roleTagText}>{m.name || '?'}</Text></View>
              ) : (
      ...
      <View style={[qs.storyBox, tBg, { borderColor: ink }]}>
        <TextInput value={story} editable={e} multiline
          onChangeText={(v) => { setStory(v); ctx.set('story', v); }}
          placeholder="they fell in like idiots because…" placeholderTextColor={ink + '77'} style={[qs.storyInput, { color: ink }]} />
      </View>
      ...
      <View style={qs.tropesWrap}>
        {tropes.map((t, i) => (
          <View key={i} style={[qs.tropeChip, { borderColor: ink }]}>
            <Text style={[qs.tropeText, { color: ink }]}>#{t}</Text>
        ...
        {e && (
          <TextInput value={tropeDraft} onChangeText={setTropeDraft} onSubmitEditing={addTrope} returnKeyType="done"
            placeholder="+ trope" placeholderTextColor={ink + '77'} style={[qs.tropeDraft, { color: ink, borderColor: ink }]} />
        )}
      </View>

      {/* song */}
      <Section title="Their song" />
      <View style={[qs.songRow, tBg, { borderColor: ink }]}>
        <View style={[qs.songIcon, { borderColor: ink }]}><Text style={[qs.songNote, { color: ink }]}>♪</Text></View>
        <View style={{ flex: 1 }}>
          <Text style={[qs.label, { color: ink }]}>song that is So Them</Text>
          <TextInput value={song} editable={e} onChangeText={(v) => { setSong(v); ctx.set('song', v); }}
            placeholder="add a track…" placeholderTextColor={ink + '77'} style={[qs.songInput, { color: ink, borderColor: ink }]} />
        </View>
      </View>
        ...
        {CHECK_LABELS.map((label, i) => {
          ...
          return (
            <Pressable key={ck} style={qs.checkRow} onPress={e ? () => setChecks({ ...checks, [ck]: !on }) : undefined} disabled={!e}>
              <View style={[qs.checkBox, { borderColor: ink }, on && { backgroundColor: ink }]}>{on && <Text style={[qs.checkMark, { color: getContrastColor(ink) }]}>✓</Text>}</View>
              <Text style={[qs.checkLabel, { color: ink }, !on && { opacity: 0.78 }]}>{label}</Text>
            </Pressable>
          );
        })}
      </View>
...
function Labeled({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <View style={qs.labeled}>
      <Text style={[qs.label, { color: ink }]}>{label}</Text>
      {children}
    </View>
  );
}
...
function Section({ title, hint }: { title: string; hint?: string }) {
  ...
  return (
    <View style={qs.section}>
      <Text style={[qs.sectionTitle, { color: ink }]}>{title}</Text>
      {hint ? <Text style={[qs.sectionHint, { color: ink }]}>{hint}</Text> : null}
    </View>
  );
}
...
const qs = StyleSheet.create({
  ...
  titlePillText: { fontFamily: FontFamily.uiSemiBold, fontSize: sf(15), letterSpacing: 0.5 },
  ...
  label: { fontFamily: FontFamily.uiSemiBold, fontSize: sf(9), letterSpacing: 0.5, textTransform: 'uppercase', color: INK },
  ...
  addChip: { borderRadius: 999, paddingVertical: 5, paddingHorizontal: 12 },
  addChipText: { fontFamily: FontFamily.uiSemiBold, fontSize: sf(12) },

  // words
  wordsRow: { flexDirection: 'row', gap: 8, marginTop: 8 },
  wordInput: { flex: 1, borderWidth: 1.5, borderColor: INK, borderRadius: 8, backgroundColor: '#fff', paddingVertical: 7, paddingHorizontal: 4, fontFamily: FontFamily.uiMedium, fontSize: sf(13), color: INK },
  ...
  meterL: { width: 74, textAlign: 'right', fontFamily: FontFamily.uiMedium, fontSize: sf(11), color: INK },
  meterR: { width: 74, fontFamily: FontFamily.uiMedium, fontSize: sf(11), color: INK },
  ...
  meterFill: { position: 'absolute', left: 0, height: 9, borderRadius: 999 },
  ...
  tropeDraft: { width: 80, borderWidth: 1.4, borderColor: '#c9a9c0', borderStyle: 'dashed', borderRadius: 999, paddingVertical: 4, paddingHorizontal: 10, fontFamily: FontFamily.ui, fontSize: sf(11), color: INK },
  ...
  songIcon: { width: 40, height: 40, borderRadius: 999, backgroundColor: '#fff', borderWidth: 1.5, borderColor: INK, alignItems: 'center', justifyContent: 'center' },
  songNote: { fontSize: sf(18), color: INK },
  ...
  checkBoxOn: {},
  checkMark: { fontSize: sf(11), fontFamily: FontFamily.uiSemiBold },
